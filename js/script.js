...
var studentSearch =
  '<div class="student-search"><input id="search" placeholder="Search for students..."><button>Search</button></div>';
...
function searchList() {
  ...
  var filteredStudents = studentItems.filter(function(i) {
    var studentEmail = $(this)
      .find(".email")
      .text();
    var studentNames = $(this)
      .find("h3")
      .text();
    if (
      studentNames.indexOf(searchTerm) > -1 ||
      studentEmail.indexOf(searchTerm) > -1
    ) {
      return true;
    }
    return false;
